...
def calcula_equilibrio_financeiro():
    gastos_essenciais = Valores.objects.filter(data_month=datetime.now().month).filter(tipo='S').filter(categoria__essencial=True)
    gastos_nao_essenciais = Valores.objects.filter(data_month=datetime.now().month).filter(tipo='S').filter(categoria__essencial=False)
